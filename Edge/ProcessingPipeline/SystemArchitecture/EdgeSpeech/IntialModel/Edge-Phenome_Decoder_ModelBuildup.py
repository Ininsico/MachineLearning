...
SAVE_SEQ = "/kaggle/working/g2p_student.pth" if IS_KAGGLE else "g2p_student.pth"
SAVE_TRANS = "/kaggle/working/g2p_refiner.pth" if IS_KAGGLE else "g2p_refiner.pth"
SEED = 42
...
meta_path = "/kaggle/input/ljsvoicedata/LJSpeech-1.1/metadata.csv"
if not os.path.exists(meta_path):
    print(f"metadata.csv not found at {meta_path}")
    print("Available files:")
    print(os.listdir(os.path.dirname(meta_path)))
    sys.exit(1)
...
loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn, num_workers=2 if IS_KAGGLE else 0)
...
def greedy_decode_seq(model, src_seq):
    model.eval()
    with torch.no_grad():
        # Handle NaN/float inputs
        if isinstance(src_seq, float):
            return [phon2idx["<eos>"]]
...
ref_loader = DataLoader(ref_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_refiner, num_workers=2 if IS_KAGGLE else 0)
...
refiner = TransformerRefiner(vocab_size=vocab_size, emb_dim=EMB_DIM, nhead=8, num_encoder_layers=3, num_decoder_layers=3, dim_feedforward=2048).to(DEVICE)
...
# Add this at the beginning (after imports)
CHECKPOINT_DIR = "/kaggle/working/checkpoints"
os.makedirs(CHECKPOINT_DIR, exist_ok=True)

# Modified train_refiner function with checkpointing
def train_refiner(resume_epoch=None):
    start_epoch = 1
    if resume_epoch:
        checkpoint_path = f"{CHECKPOINT_DIR}/refiner_epoch_{resume_epoch}.pth"
        if os.path.exists(checkpoint_path):
            checkpoint = torch.load(checkpoint_path)
            refiner.load_state_dict(checkpoint['model_state'])
            opt_ref.load_state_dict(checkpoint['optimizer_state'])
    ...
    for epoch in range(start_epoch, EPOCHS_TRANS + 1):
        total_loss = 0.0
        total_per = 0.0
        steps = 0
        t0 = time.time()
        
        batch_iter = tqdm(ref_loader, desc=f"Refiner Epoch {epoch}")
        for i, (src, src_lens, tgt, tgt_lens) in enumerate(batch_iter):
            src, src_lens, tgt = src.to(DEVICE), src_lens.to(DEVICE), tgt.to(DEVICE)
            sos = torch.full((tgt.size(0), 1), phon2idx["<sos>"], dtype=torch.long, device=DEVICE)
            dec_inp = torch.cat([sos, tgt[:, :-1]], dim=1)
            ...
            if i % 100 == 0:
                checkpoint = {
                    ...
                    'optimizer_state': opt_ref.state_dict(),
                    'loss': loss.item(),
                }
                torch.save(checkpoint, f"{CHECKPOINT_DIR}/refiner_epoch_{epoch}_batch_{i}.pth")
        ...
        checkpoint = {
            ...
            'model_state': refiner.state_dict(),
            'optimizer_state': opt_ref.state_dict(),
            'loss': total_loss/len(ref_loader),
        }
        torch.save(checkpoint, f"{CHECKPOINT_DIR}/refiner_epoch_{epoch}.pth")
        
        avg_loss = total_loss/len(ref_loader)
        print(f"Refiner Epoch {epoch} loss {avg_loss:.4f} time {time.time()-t0:.1f}s")
...
def find_latest_checkpoint():
    ...
    for f in os.listdir(CHECKPOINT_DIR):
        if f.startswith("refiner_epoch_") and f.endswith(".pth"):
            try:
                ...
                continue
    if checkpoints:
        latest_epoch = max([x[0] for x in checkpoints])
        latest_checkpoint = [x[1] for x in checkpoints if x[0] == latest_epoch][0]
        return latest_epoch, f"{CHECKPOINT_DIR}/{latest_checkpoint}"
    return None, None
...
if latest_checkpoint:
    ...
    refiner.load_state_dict(checkpoint['model_state'])
    opt_ref.load_state_dict(checkpoint['optimizer_state'])
    ref_losses = train_refiner(resume_epoch=latest_epoch)
else:
